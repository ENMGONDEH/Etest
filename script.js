...
function showStatusPage() {
    ...
    coinsElement.innerText = `تعداد سکه: ${coins}`;

    const clickRateElement = document.createElement('p');
    clickRateElement.innerText = `نرخ کلیک: ${clickRate} بر ثانیه`;

    const clickValueElement = document.createElement('p');
    clickValueElement.innerText = `میزان سکه در هر کلیک: ${clickValue}`;

    const upgradeClickLevelElement = document.createElement('p');
    upgradeClickLevelElement.innerText = `سطح ارتقا نرخ کلیک: ${upgradeClickLevel}`;

    const upgradeValueLevelElement = document.createElement('p');
    upgradeValueLevelElement.innerText = `سطح ارتقا میزان سکه در کلیک: ${upgradeValueLevel}`;

    statusContainer.appendChild(closeButton);
    statusContainer.appendChild(coinsElement);
    statusContainer.appendChild(clickRateElement);
    statusContainer.appendChild(clickValueElement);
    statusContainer.appendChild(upgradeClickLevelElement);
    statusContainer.appendChild(upgradeValueLevelElement);

    document.body.appendChild(statusContainer);
}

document.getElementById('coinButton').addEventListener('click', handleClick);